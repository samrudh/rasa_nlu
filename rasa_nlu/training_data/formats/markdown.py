...
class MarkdownReader(TrainingDataReader):
    ...
    def _find_section_header(self, line):
        """Checks if the current line contains a section header and
        returns the section and the title."""
    ...
    def _find_entities_in_training_example(self, example):
        ...
        for match in re.finditer(ent_regex, example):
            ...
            entity_value = (match.groupdict()['value']
                            if match.groupdict()['value'] else entity_text)
...
class MarkdownWriter(TrainingDataWriter):
    ...
    def _generate_training_examples_md(self, training_data):
        ...
        training_examples = sorted([e.as_dict() for e
                                    in training_data.training_examples],
                                   key=lambda k: k['intent'])
        md = u''
        for i, example in enumerate(training_examples):
            if (i == 0 or
                    training_examples[i - 1]['intent'] != example['intent']):
